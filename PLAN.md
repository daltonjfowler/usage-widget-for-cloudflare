...
- Version 1.2 gate is PENDING a machine with the Android SDK. The 1.2 session ran on a box with only JDK 25 and no Android SDK, so `gradlew testDebugUnitTest` could not run (Gradle 8.11.1 also predates JDK 25). The new day-to-day math in `History` — daily deltas, cycle-reset handling, missing-day vs baseline, currency selection, and the 40-day cap — was verified standalone against JDK 25 with a verbatim copy of the pure logic: 26/26 assertions passed. `HistoryTest` (16 cases) mirrors these plus serialize/read round-trips and awaits the full suite on the build machine. The Robolectric preview PNGs were NOT re-rendered for 1.2 and are stale; regenerate them and the `docs/`/`releases/` screenshots on a machine with the SDK before release. Dalton to run the full gate and re-export images.
